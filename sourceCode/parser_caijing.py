...
def minorRandomPause():
    ...
    lw.log_writer('财经网脚本进入休眠' + str(randomTime) + '秒')
...
def parsingContent(link, title):
    ...
    try:
        ...
        lw.log_writer('财经网' + fullLink + '失败')
        return {'news_link': fullLink.strip(), 'news_title': title.strip(), 'news_source': '财经网', 'news_content': content.strip(), 'news_date': news_date}

    lw.log_writer('财经网开始爬取' + fullLink)
...
def main():
    ...
    try:
        r = requests.get('https://www.caijing.com.cn/')
    ...
    news_list = soup.find('div', {'class':'yaowen'}).findAll('li')
    news_list_item = {}
    ...
    for a in news_list_item:
        try:
            ...
            lw.log_writer('财经网添加新闻错误')
            pass
    
    lw.log_writer('财经网本轮新增新闻有' + str(len(confirmed_new)) + '条')
    ...
    if len(confirmed_new) == 0:
        ...
        for link in confirmed_new:
            ...
            try:
                ...
                lw.log_writer('财经网在添加数据时失败')

            lw.log_writer('财经网新增' + str(mycursor.rowcount) + '条')
            minorRandomPause()

        lw.log_writer('财经网本轮结束')
        mydb.close()
