...
import task_topicTrend as task_tt
import task_removeDuplicate as task_rd
# ============== 日常计算任务 =================
schedule.every().day.at("23:30").do(task_gsc.main) # 政府计算器
schedule.every().day.at("23:30").do(task_csc.main) # 公司计算器

schedule.every().hour.do(task_rd.main) # 去重

schedule.every().hour.do(task_tagging.tagging) # 在bridge打标签

schedule.every().day.at("23:15").do(task_tt.main) # 话题走势
