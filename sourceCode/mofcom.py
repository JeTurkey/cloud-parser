...
def parsingContent(link):
    ...
    rst = {'urlLink': link, 'title': title, 'source': '光明网财经', 'content': content, 'dateAdded': dateFormat, 'timeAdded': timeFormat}
        ...
        for link in links:
            ...
            for item in tmp:
                if 'http' in item.find('a').get('href'):
                    ...
                    results.append('http://www.mofcom.gov.cn' + item.find('a').get('href'))
